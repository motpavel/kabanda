...
test('legacy-server compatibility: owner completes one canonical raid and opens the next raid form', async ({ context, page }, testInfo) => {
  ...
  await page.setViewportSize({ width: 390, height: 844 })
  const actionsTrigger = page.getByRole('button', { name: 'Действия рейда' })
  await expect(actionsTrigger).toHaveCSS('padding', '0px')
  ...
  await expect(actionsDialog.getByRole('button')).toHaveText(['', 'Поставить на паузу', 'Сменить навигатора', 'Завершить рейд'])
  ...
  await expect(nearbyMarker).toHaveCSS('width', '24px')
  await expect(nearbyMarker).toHaveCSS('height', '24px')
  ...
  await expect(nearbyMarker).toHaveCSS('border-top-width', '6px')
  ...
  await arrivalSheet.locator('summary').click()
  await expect(arrivalSheet.getByRole('group', { name: 'Кто сейчас здесь?' })).toBeVisible()
  await expect(arrivalSheet.getByText('Ещё одно место в истории вашей Кабанды.')).toHaveCount(0)
  await expect(arrivalSheet.locator('.checkin-participant__name small')).toHaveCount(0)
  await expect(arrivalSheet.getByRole('button', { name: 'Пометить точку', exact: true })).toBeInViewport()
  await page.setViewportSize({ width: 320, height: 740 })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await expect(arrivalSheet.getByRole('button', { name: 'Пометить точку', exact: true })).toBeInViewport()
  await page.screenshot({ path: testInfo.outputPath('point-check-in-320.png'), animations: 'disabled' })
  await page.setViewportSize({ width: 390, height: 844 })
  ...
  await expect(visitedMarker).toHaveCSS('height', '18px')
  await expect(visitedMarker).toHaveCSS('border-radius', '50%')
  await expect(visitedMarker).toHaveCSS('border-top-width', '5px')
  await expect(visitedMarker).toHaveCSS('border-top-color', 'rgb(133, 214, 154)')
  await visitedMarker.hover()
  await expect(visitedMarker).toHaveCSS('transform', 'matrix(1, 0, 0, 1, -9, -9)')
  await expect(visitedMarker).toHaveCSS('border-top-color', 'rgb(133, 214, 154)')
  await visitedMarker.click()
  const historySheet = page.getByRole('complementary', { name: 'Точка: Синтетическая точка E2E' })
  await expect(historySheet.getByRole('button', { name: 'Вы 1 раз', exact: true })).toBeVisible()
  expect(fixture<{ agedVisits: number }>('age-point-visit', raid.id).agedVisits).toBe(1)
  await historySheet.getByRole('button', { name: 'Пометить точку снова' }).click()
  await context.setGeolocation({ latitude: 56.86015, longitude: 53.21015, accuracy: 8 })
  await arrivalSheet.getByRole('button', { name: 'Пометить точку' }).click()
