...
const environmentSchema = z.object({
  ...
  if (value.SMTP_SECURE && value.SMTP_REQUIRE_TLS) {
    context.addIssue({ code: 'custom', path: ['SMTP_REQUIRE_TLS'], message: 'Use SMTP_SECURE for implicit TLS or SMTP_REQUIRE_TLS for STARTTLS, not both' })
  }
  if (value.SMTP_USER && !value.SMTP_SECURE && !value.SMTP_REQUIRE_TLS) {
    context.addIssue({ code: 'custom', path: ['SMTP_REQUIRE_TLS'], message: 'Authenticated SMTP requires implicit TLS or required STARTTLS' })
  }
  if (value.ALPHA_ACCESS_MODE === 'enforced' && !value.ALPHA_ACCESS_SECRET) {
    context.addIssue({ code: 'custom', path: ['ALPHA_ACCESS_SECRET'], message: 'ALPHA_ACCESS_SECRET is required when closed-alpha access is enforced' })
  }
})

export type ApiConfig = z.infer<typeof environmentSchema> & { cookieName: string; secureCookies: boolean }
export function loadConfig(environment: NodeJS.ProcessEnv = process.env): ApiConfig {
  const parsed = environmentSchema.parse(environment)
  const appOrigin = new URL(parsed.APP_ORIGIN)
  if (parsed.NODE_ENV === 'production' && parsed.MEDIA_CAPABILITY_SECRET === localMediaCapabilitySecret) {
  ...
  return { ...parsed, APP_ORIGIN: appOrigin.origin, secureCookies, cookieName: secureCookies ? '__Host-kabanda_session' : 'kabanda_session' }
}
