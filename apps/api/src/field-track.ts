...
export async function readRouteChanges(pool: Pool, userId: string, raidId: string, after: string, previousEpoch?: string) {
  ...
  try {
    ...
    const windows = await client.query(
      'SELECT opened_at, closed_at FROM raid_activity_windows WHERE raid_id=$1 ORDER BY opened_at,id', [raidId],
    )
    const cutoffs = await client.query(
      'SELECT lease_id,max_sequence::text FROM raid_route_finalization_cutoffs WHERE raid_id=$1 ORDER BY lease_id', [raidId],
    )
    ...
    const page = await client.query<{ sync_ordinal: string; lease_id: string; sequence: string }>(
      `SELECT sync_ordinal::text,lease_id,sequence::text FROM raid_route_samples
       WHERE raid_id=$1 AND sync_ordinal>$2::bigint ORDER BY sync_ordinal LIMIT 151`, [raidId, cursor],
    )
    const selected = page.rows.slice(0, 150)
    // A late sequence can repair the edge of a point delivered on an earlier
    // page. Return that successor too; clients upsert by lease/sequence rather
    // than appending coordinates blindly.
    ...
    return {
      schemaVersion: 1 as const, raidId, epoch, reset,
      cursor: selected.at(-1)?.sync_ordinal ?? cursor,
      hasMore: page.rows.length > selected.length,
      serverAt: access.server_at.toISOString(),
