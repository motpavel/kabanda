...
import type { CheckInResponse, OneShotCoordinate } from './types'
...
export function TeamVisitPanel(props: {
  ...
  const prepared = useRef<OneShotCoordinate | null>(null)
  const preparedRequest = useRef<Promise<OneShotCoordinate> | null>(null)
  ...
  useEffect(() => {
    ...
    setSelected(previous => {
      const next = participantSelectionAfterPresenceRefresh({ identityId, selectedParticipantIds: previous,
        ...
        freezeSuggestions: sending })
      return previous.join(':') === next.join(':') ? previous : next
    })
  }, [identityId, point, positions, activeIds, sending])

  const locate = () => {
    if (preparedRequest.current) return preparedRequest.current
    const request = getOneShotCoordinate(10_000).then(coordinate => {
      if (mounted.current) prepared.current = coordinate
      return coordinate
    }).finally(() => { if (preparedRequest.current === request) preparedRequest.current = null })
    preparedRequest.current = request
    return request
  }
  useEffect(() => {
    if (!visible) return
    const warm = () => { if (document.visibilityState === 'visible') void locate().catch(() => undefined) }
    warm()
    const interval = setInterval(warm, 5000)
  ...
  const submit = async () => {
    if (busy || sending || raid.state !== 'active' || raid.navigatorUserId !== identityId || !activeIds.has(identityId)) return
    ...
    try {
      const saved = prepared.current
      const age = saved ? Date.now() - Date.parse(saved.capturedAt) : Infinity
      const evidence = saved && age >= -5000 && age <= 10_000 ? saved : await locate()
      ...
      // This lane does not wait for photos, gallery reads or the route history.
