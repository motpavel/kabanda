...
import { selectRiderMarkers, riderDistanceMeters } from './rider-markers'
...
describe('participant and navigator map markers', () => {
  it('shows a white participant and a red navigator beyond 50 m', () => {
    const markers = selectRiderMarkers(input(50.1))
    expect(markers.map(marker => marker.kind)).toEqual(['participant', 'navigator'])
    expect(riderDistanceMeters(markers[0]!.point, markers[1]!.point)).toBeCloseTo(50.1)
  })
  it.each([0, 20, 49.9, 50])('merges %s m into one flock at the participant coordinate', (meters) => {
    const data = input(meters)
    expect(selectRiderMarkers(data)).toEqual([expect.objectContaining({ id: 'flock', kind: 'flock', point: data.location })])
  })
  it('splits again after leaving the radius and merges when returning', () => {
    expect(selectRiderMarkers(input(49))).toHaveLength(1)
    expect(selectRiderMarkers(input(51))).toHaveLength(2)
    expect(selectRiderMarkers(input(49))).toHaveLength(1)
  })
