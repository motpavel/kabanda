-- Additive field protocol. Never rewrites GPS payloads, existing receipts or
-- immutable results. This migration has not been applied to a working server.
...
CREATE TABLE raid_sync_revisions (
  ...
  points_revision bigint NOT NULL DEFAULT 0 CHECK (points_revision >= 0)
);
CREATE FUNCTION bump_raid_sync_revision() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE target uuid; points_changed bigint := 0;
BEGIN
  ...
  IF TG_TABLE_NAME IN ('raid_point_snapshots','raid_point_credits','raid_point_visit_events') THEN points_changed := 1; END IF;
  IF EXISTS(SELECT 1 FROM raids WHERE id=target) THEN
    INSERT INTO raid_sync_revisions(raid_id,revision,points_revision) VALUES(target,1,points_changed)
      ON CONFLICT(raid_id) DO UPDATE SET revision=raid_sync_revisions.revision+1,
        points_revision=raid_sync_revisions.points_revision+points_changed;
  END IF;
  RETURN NULL;
END $$;
...
CREATE TRIGGER raid_points_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_point_snapshots FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
...
-- The existing batch writer serializes inserts on the raid row. This ordinal
-- therefore permits incremental reads even when a late batch fills an older
-- GPS sequence. Sequence/capturedAt/evidence themselves are not changed.
ALTER TABLE raid_route_samples ADD COLUMN sync_ordinal bigint GENERATED ALWAYS AS IDENTITY;
CREATE UNIQUE INDEX raid_route_samples_sync_idx ON raid_route_samples(raid_id,sync_ordinal);
