-- Additive field-sync protocol. Existing receipts, queues, media and frozen
-- results keep their meaning. No data is deleted or historical result edited.
...
BEGIN
  ...
  RETURN NULL;
END $$;
CREATE TRIGGER raids_sync_revision AFTER INSERT OR UPDATE ON raids
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_participants_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_participants
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_credits_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_point_credits
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_visits_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_point_visit_events
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_claims_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_checkin_claims
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_fallbacks_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_checkin_fallbacks
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
CREATE TRIGGER raid_presence_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_presence_reports
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();

-- A separate material record is deliberately not included in frozen metrics.
-- A photograph/comment added after a visit or finish cannot create a credit.
CREATE TABLE raid_point_materials (
  ...
  declared_type text CHECK(declared_type IN ('image/jpeg','image/png','image/webp')),
  ...
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  UNIQUE(author_user_id, operation_id),
  CHECK((kind='comment' AND char_length(body)>0 AND ready AND content_bytes IS NULL)
    OR (kind='photo' AND source_sha256 IS NOT NULL AND declared_type IS NOT NULL AND declared_size IS NOT NULL
      AND (NOT ready OR (octet_length(content_bytes) BETWEEN 1 AND 3145728
        AND width BETWEEN 1 AND 2048 AND height BETWEEN 1 AND 2048))))
);
CREATE INDEX raid_point_materials_list_idx ON raid_point_materials(raid_id,point_snapshot_id,created_at,id) WHERE ready;
CREATE TRIGGER raid_materials_sync_revision AFTER INSERT OR UPDATE OR DELETE ON raid_point_materials
  FOR EACH ROW EXECUTE FUNCTION bump_raid_sync_revision();
