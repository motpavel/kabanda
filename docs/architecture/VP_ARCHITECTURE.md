...
- В закрытой альфе восстановление invite-аккаунта выполняет оператор: старый пароль не читается и не
  возвращается, вместо него задаётся новый. Команда работает только из exact release, по точному логину,
  с dry-run и буквальным подтверждением; после смены хэша все серверные сессии identity отзываются.
